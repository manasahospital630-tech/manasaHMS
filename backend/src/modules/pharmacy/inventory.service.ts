...
export const getSalesHistory = async () => {
  ...
  for (const sale of sales) {
    ...
    if (saleDate >= startOfDay && saleDate <= endOfDay) updateStats(stats.day);
    if (saleDate >= startOfYesterday && saleDate <= endOfYesterday) updateStats(stats.yesterday);
    if (saleDate >= startOfWeek) updateStats(stats.week);
    if (saleDate >= startOfLastWeek && saleDate <= endOfLastWeek) updateStats(stats.lastWeek);
    if (saleDate >= startOfMonth) updateStats(stats.month);
    if (saleDate >= startOfLastMonth && saleDate <= endOfLastMonth) updateStats(stats.lastMonth);
    if (saleDate >= startOfYear) updateStats(stats.year);
  }
