...
const createUser = async (input) => {
    ...
    const existing = await (0, database_1.query)('SELECT user_id FROM users WHERE LOWER(email) = $1', [cleanEmail]);
...
const getHospitalSettings = async () => {
    let result = await (0, database_1.query)('SELECT * FROM hospital_settings WHERE id = 1');
    if (result.rows.length === 0) {
        await (0, database_1.query)(`INSERT INTO hospital_settings (id, hospital_name, hospital_address, phone_number, website, email, gstin, license_info)
       ...
       ON DUPLICATE KEY UPDATE hospital_name = VALUES(hospital_name)`);
        result = await (0, database_1.query)('SELECT * FROM hospital_settings WHERE id = 1');
    }
    return result.rows[0];
};
...
const updateHospitalSettings = async (input) => {
    ...
    if (input.hospitalLogo && input.hospitalLogo.startsWith('data:image/')) {
        try {
            logoUrl = await (0, s3Upload_1.uploadBase64Image)(input.hospitalLogo, 'images');
    ...
    // Ensure row 1 exists
    await (0, exports.getHospitalSettings)();
    await (0, database_1.query)(`UPDATE hospital_settings
     SET hospital_name = $1,
         hospital_address = $2,
         phone_number = $3,
         website = $4,
    ...
    return (0, exports.getHospitalSettings)();
};
...
const getDashboardStats = async () => {
    try {
        const [docTotalRes, presentDocsRes, nurseTotalRes, otherStaffRes, opBookedRes, revOverallRes, billsTotalRes, revTodayRes, ipBillsRes, bedsTotalRes, bedsAvailRes, bedsOccRes, activityRes] = await Promise.all([
            ...
            (0, database_1.query)("SELECT COUNT(*) as count FROM users WHERE role IN ('Receptionist', 'Pharmacist', 'Biller', 'Patient', 'Management') AND is_active = TRUE"),
            (0, database_1.query)("SELECT COUNT(*) as count FROM appointments WHERE DATE(appointment_date) = CURRENT_DATE()"),
            (0, database_1.query)("SELECT COALESCE(SUM(total_amount), 0) as count FROM billing_invoices WHERE status = 'Paid'"),
            (0, database_1.query)("SELECT COUNT(*) as count FROM billing_invoices"),
            (0, database_1.query)("SELECT COALESCE(SUM(total_amount), 0) as count FROM billing_invoices WHERE DATE(created_at) = CURRENT_DATE() AND status = 'Paid'"),
            (0, database_1.query)("SELECT COUNT(*) as count FROM billing_invoices"),
