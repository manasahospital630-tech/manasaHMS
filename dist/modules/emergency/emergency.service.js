...
const admitEmergencyPatient = async (input) => {
    ...
    `, [
        ...
        if (doctorId) {
            const docCheck = await (0, database_1.query)('SELECT user_id FROM users WHERE user_id = $1', [doctorId]);
            if (docCheck.rows.length === 0) {
                doctorId = null;
            }
        }
        if (!doctorId) {
            const fallbackDoc = await (0, database_1.query)("SELECT user_id FROM users WHERE role IN ('Doctor', 'Admin') ORDER BY created_at ASC LIMIT 1");
