...
export const generateMRN = async (): Promise<string> => {
  ...
  try {
    const result = await query(
      `SELECT mrn FROM patients WHERE mrn LIKE $1 ORDER BY created_at DESC LIMIT 1`,
      [`${prefix}%`]
    );
  ...
  try {
    let exists = await query(`SELECT patient_id FROM patients WHERE mrn = $1`, [finalMrn]);
    let attempts = 0;
    while (exists.rows.length > 0 && attempts < 100) {
      ...
      exists = await query(`SELECT patient_id FROM patients WHERE mrn = $1`, [finalMrn]);
    }
  } catch (err) {
